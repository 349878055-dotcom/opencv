...
_health_ok() {
  curl -sf --max-time 2 "$HEALTH" 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); exit(0 if d.get('ok') and d.get('version',0)>=11 else 1)" 2>/dev/null
}
...
echo "⏳ 能量工作台 v12（独立模式 · 已脱离 ComfyUI）"
_stop_port
_start_server
