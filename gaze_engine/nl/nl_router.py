...
from gaze_engine.nl.nl_intent import (
    INTENT_APPLY,
    INTENT_CONSULT,
    CustomerNLResult,
...
from gaze_engine.nl.nl_to_packet import _packet_keyword_fallback, match_preset_from_text
from gaze_engine.input.slider_schema import SliderPacket

def load_internal_knowledge() -> str:
    """厂内知识库（客户画布不填）；来自已保存上下文或空。"""
    try:
        from gaze_engine.delivery.workbench_context import read_workbench_context
...
def _consult_reply_keyword(text: str) -> str:
    ...
    if "预设" in t or "情绪" in t:
        ...
        names = "、".join(list(presets.keys())[:6]) + "…"
