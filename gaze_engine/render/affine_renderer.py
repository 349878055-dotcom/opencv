...
用法:
  # 渲染单帧
  python -c "from gaze_engine.render.affine_renderer import AffineRenderer; r=AffineRenderer(); img=r.render_frame(channels_dict)"
...
from gaze_engine.render.affine_gloss import draw_eye_gloss
...
TEXTURE_PATH = _PKG / "render" / "assets" / "eyelid_raw.png"
...
# 为 _DEFAULT_CONSTANTS 补充标准眉位偏移（仅 fallback 场景使用）
# 生产场景下，BROW_*_OFF 由 apply_render_baseline() 从 MediaPipe 注入
if "BROW_INNER_OFF" not in _DEFAULT_CONSTANTS:
    _DEFAULT_CONSTANTS["BROW_INNER_OFF"] = (-130, -90)
    _DEFAULT_CONSTANTS["BROW_PEAK_OFF"] = (0, -115)
    _DEFAULT_CONSTANTS["BROW_OUTER_OFF"] = (130, -90)
if "BLINK_DROP" not in _DEFAULT_CONSTANTS:
    _DEFAULT_CONSTANTS["BLINK_DROP"] = 40
    _DEFAULT_CONSTANTS["SQUINT_LIFT"] = 25
    _DEFAULT_CONSTANTS["BROW_DOWN"] = 30
    _DEFAULT_CONSTANTS["BROW_RAISE_AMP"] = 25
...
class EyeMesh:
    ...
    def __init__(
        ...
        pupil_rest: tuple[int, int] = (0, 0),
    ):
        self.cx = cx
        self.cy = cy
        self.side = side
        self.pupil_rest = pupil_rest
        self.eye_w = eye_w
    ...
    def deform(
        ...
        *,
        ...
        rest_dx, rest_dy = self.pupil_rest
        pupil_cx = int(cx + rest_dx + px * pupil_x_range * pupil_s)
        pupil_cy = int(cy + rest_dy + py * pupil_y_range * pupil_s)
...
if _AFFINE_DISABLED:
    ...
    class AffineRenderer:
        ...
        spatial_calibration 非空时：1024 画布经仿射投影到 690×361（叠加预览）；
        否则等比缩放（扩散引擎 MP4）。
        """
        def __init__(
            self,
            ...
            self.spatial = spatial_calibration

            # 校验必填常量（生产路径：template_constants 由 apply_render_baseline 注入）
            _REQUIRED_KEYS = [
                "LEFT_CX", "LEFT_CY", "RIGHT_CX", "RIGHT_CY",
                "EYE_W", "UPPER_PEAK", "LOWER_BOT",
                "PUPIL_R_BASE", "IRIS_R_BASE",
                "BLINK_DROP", "SQUINT_LIFT",
                "BROW_DOWN", "BROW_RAISE_AMP",
                "LID_UPPER_DROP", "LID_LOWER_LIFT",
                "BROW_THICK", "EYELID_THICK", "PUPIL_THICK",
                "PUPIL_X_RANGE", "PUPIL_Y_RANGE",
                "IRIS_SCALE_RANGE", "PUPIL_SCALE_RANGE",
            ]
            missing = [k for k in _REQUIRED_KEYS if k not in c]
            if missing:
                raise KeyError(
                    f"AffineRenderer 缺少必填渲染常量: {missing}. "
                    "请确保 template_constants 由 template_to_renderer_constants() "
                    "配合 apply_render_baseline() 生成。"
                )

            def _brow_off(cmap: dict, side: str, part: str, default_key: str) -> tuple:
                key = f"BROW_{side}_{part.upper()}_OFF"
                if key in cmap:
                    return tuple(cmap[key])
                if default_key in cmap:
                    return tuple(cmap[default_key])
                # 无眉位数据时使用标准底膜默认值（仅在缺少 render_baseline 时触发）
                _STANDARD_BROWS = {
                    "BROW_INNER_OFF": (-130, -90),
                    "BROW_PEAK_OFF": (0, -115),
                    "BROW_OUTER_OFF": (130, -90),
                }
                if default_key in _STANDARD_BROWS:
                    return _STANDARD_BROWS[default_key]
                raise KeyError(
                    f"Brow offset missing: neither '{key}' nor '{default_key}' found. "
                    "Ensure apply_render_baseline() was called with a valid render_baseline."
                )
            ...
            self.meshes = [
                EyeMesh(c["LEFT_CX"], c["LEFT_CY"], -1,
                        ...
                        brow_inner_off=_brow_off(c, "LEFT", "inner", "BROW_INNER_OFF"),
                        brow_peak_off=_brow_off(c, "LEFT", "peak", "BROW_PEAK_OFF"),
                        brow_outer_off=_brow_off(c, "LEFT", "outer", "BROW_OUTER_OFF"),
                        pupil_rest=tuple(c.get("PUPIL_REST_LEFT") or (0, 0))),
                EyeMesh(c["RIGHT_CX"], c["RIGHT_CY"], 1,
                        eye_w=c["EYE_W"],
                        upper_peak=c["UPPER_PEAK"],
                        lower_bot=c["LOWER_BOT"],
                        brow_inner_off=_brow_off(c, "RIGHT", "inner", "BROW_INNER_OFF"),
                        brow_peak_off=_brow_off(c, "RIGHT", "peak", "BROW_PEAK_OFF"),
                        brow_outer_off=_brow_off(c, "RIGHT", "outer", "BROW_OUTER_OFF"),
                        pupil_rest=tuple(c.get("PUPIL_REST_RIGHT") or (0, 0))),
            ]
            self.sx, self.sy = _calc_scale()

        def _project_output(self, canvas: np.ndarray) -> np.ndarray:
            """1024 模型画布 → 输出分辨率；有空间标定时仿射投影，否则等比缩放。"""
            if self.spatial is not None:
                return self.spatial.warp(canvas)
            return cv2.resize(canvas, (OUTPUT_W, OUTPUT_H), interpolation=cv2.INTER_AREA)
        ...
        def render_frame(self, channels: dict[str, float]) -> np.ndarray:
            ...
            return self._project_output(canvas)
