...
| 文件 | 关系 |
|------|------|
| [`全量帧指令集规范.md`](../01_总纲/全量帧指令集规范.md) | 下游：定义 12 通道 × 150 帧的工程真值 |
| [`节奏说明书.md`](../01_总纲/节奏说明书.md) | 下游：定义 05_扩散节拍表.txt 的格式 |
| [`眼眉指令集_全局情绪节奏主钟.md`](../01_总纲/眼眉指令集_全局情绪节奏主钟.md) | 下游：定义 12 通道指令集协议 |
| [`流程设计.md`](流程设计.md) | 上层：双模驱动管线架构 |
| [`工程底膜合同.md`](../03_工程底膜/工程底膜合同.md) | 下游：定义线条图渲染协议——工程底膜是 OpenCV 渲染的 150 帧 RGB 三色分离 PNG/MP4 |
| [`工程底膜驱动规范.md`](../03_工程底膜/工程底膜驱动规范.md) | 下游：定义 affine_renderer 驱动规范 |
...
| ❌ 不管 | 原因 |
|---------|------|
| 12 通道指令集的数值生成 | 见 [`全量帧指令集规范.md`](../01_总纲/全量帧指令集规范.md) |
| 节奏说明书（05_节拍表.txt）的生成 | 见 [`节奏说明书.md`](../01_总纲/节奏说明书.md)，由 [`rhythm_compiler.py`](../../gaze_engine/_shared/rhythm_compiler.py) 实现 |
| 工程底膜/线条图的渲染 | 见 [`工程底膜合同.md`](../03_工程底膜/工程底膜合同.md)，由 [`affine_renderer.py`](../../gaze_engine/human/affine_renderer.py) 实现 |
...
│    · 02_烘焙.json → 03_工程底模.mp4│
│      (affine_renderer 渲染)       │
│    · 05_节拍表 + 叙事 + 元信息     │
│      → 04_Prompt.txt             │
│    · 同一 revision 绑定 MP4 + 04  │
│                                  │
│  输出（送扩散引擎）:               │
│    ① OpenCV 线条图 MP4           │
│    ② 04_给视频生成的Prompt.txt   │
...
```json
{
  "video": "opencv_线条图.mp4",        // → 拼入最终 payload 的 video 字段
  "prompt_04": "# 给视频生成引擎的说明\n\n...",  // → 拼入最终 payload 的 prompt 字段
  "payload": {
    "video": "opencv_线条图.mp4 路径",
...
def assemble(baked_json, customer_action, metadata):
    ...
    # 2. 调用 affine_renderer 渲染线条图 MP4
    line_art_video = render_affine(baked_json, species=metadata.species)
    #    ↑ 已有代码, 来自 gaze_engine/*/affine_renderer.py

    # 3. 拼装 04_给视频生成的Prompt.txt
...
"""
    ...
    # 4. 返回送扩散引擎的最终 payload
    return {
        "video": line_art_video,       # ① OpenCV 线条图 MP4
        "prompt": prompt_04,            # ② 04_Prompt.txt 内容
    }
```
...
| 2026-05-25 | 明确最终输出为 OpenCV 线条图 MP4 + 04_Prompt.txt，非 JSON | 确认：送扩散引擎的是线条图 MP4 和文本 Prompt，中间文件 02_烘焙.json 不送出 |