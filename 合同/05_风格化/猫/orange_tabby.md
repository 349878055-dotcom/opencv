...
> **类型：个性化 · 猫 · 单项** · id=`orange_tabby`

> **状态：🧠 脑补初稿（可逐份审定）** — 本文 **只管辖「橘猫/贪玩型」**（id=`orange_tabby`）；不与其它品种合并修订。
> **数值真源**：`预设资产/风格包/cat/orange_tabby/style.json` · schema `ecursor_style_v1`
...
定义 **橘猫/贪玩型** 的 **12 通道 base_offset / scale_factor**（及几何偏移如有），描述气质与验收。  
...
| label | 橘猫/贪玩型 |
| species | `cat` |
| notes | 常见橘白，圆眼机警贪玩，扫视略多， blink 少而快 |
...
| 通道 | 中文 | base | scale | 🧠 意图 |
|------|------|------|-------|--------|
| `pupil_x` | 瞳孔水平 | **0.55** | **0.1** | 扫视/回头幅度；接近物种默认 |
| `pupil_y` | 瞳孔垂直 | **0.52** | **0.09** | 抬眼/低眉视线；接近物种默认 |
| `blink` | 眨眼 | **0.4** | **0.18** | 眼睑开合动态；基线偏低 |
| `eyebrow` | 眉形整体 | **0.54** | **0.14** | 眉弓压抬；接近物种默认 |
| `pupil_scale` | 瞳孔缩放 | **0.56** | **0.2** | 惊恐/聚焦；动态幅度大 |
| `iris_scale` | 虹膜缩放 | **0.52** | **0.14** | 眼内圈大小；接近物种默认 |
| `cornea_bulge` | 角膜鼓胀 | **0.52** | **0.16** | 湿润/受光；接近物种默认 |
| `squint` | 眯眼 | **0.42** | **0.18** | 笑眼/不适；基线偏低 |
| `brow_raise` | 挑眉 | **0.52** | **0.14** | 警觉/疑问；接近物种默认 |
| `lid_upper` | 上睑 | **0.46** | **0.14** | 睁眼度；接近物种默认 |
| `lid_lower` | 下睑 | **0.46** | **0.14** | 眼下缘；接近物种默认 |
| `eye_gloss` | 高光 | **0.54** | **0.06** | 泪膜/湿润感；动态幅度小 |
...
`预设资产/风格包/cat/orange_tabby/style.json`
