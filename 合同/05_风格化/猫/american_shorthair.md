...
> **类型：个性化 · 猫 · 单项** · id=`american_shorthair`

> **状态：🧠 脑补初稿（可逐份审定）** — 本文 **只管辖「美短/活泼型」**（id=`american_shorthair`）；不与其它品种合并修订。
> **数值真源**：`预设资产/风格包/cat/american_shorthair/style.json` · schema `ecursor_style_v1`
...
定义 **美短/活泼型** 的 **12 通道 base_offset / scale_factor**（及几何偏移如有），描述气质与验收。  
...
| 通道 | 中文 | base | scale | 🧠 意图 |
|------|------|------|-------|--------|
| `pupil_x` | 瞳孔水平 | **0.48** | **0.05** | 扫视/回头幅度；动态幅度小 |
| `pupil_y` | 瞳孔垂直 | **0.47** | **0.05** | 抬眼/低眉视线；动态幅度小 |
| `blink` | 眨眼 | **0.46** | **0.2** | 眼睑开合动态；动态幅度大 |
| `eyebrow` | 眉形整体 | **0.48** | **0.08** | 眉弓压抬；接近物种默认 |
| `pupil_scale` | 瞳孔缩放 | **0.5** | **0.14** | 惊恐/聚焦；接近物种默认 |
| `iris_scale` | 虹膜缩放 | **0.5** | **0.1** | 眼内圈大小；接近物种默认 |
| `cornea_bulge` | 角膜鼓胀 | **0.5** | **0.12** | 湿润/受光；接近物种默认 |
| `squint` | 眯眼 | **0.44** | **0.16** | 笑眼/不适；接近物种默认 |
| `brow_raise` | 挑眉 | **0.48** | **0.08** | 警觉/疑问；接近物种默认 |
| `lid_upper` | 上睑 | **0.46** | **0.12** | 睁眼度；接近物种默认 |
| `lid_lower` | 下睑 | **0.46** | **0.12** | 眼下缘；接近物种默认 |
| `eye_gloss` | 高光 | **0.5** | **0.05** | 泪膜/湿润感；动态幅度小 |
...
`预设资产/风格包/cat/american_shorthair/style.json`
