...
> **类型：个性化 · 猫 · 单项** · id=`siamese_cat`

> **状态：🧠 脑补初稿（可逐份审定）** — 本文 **只管辖「暹罗猫/高冷型」**（id=`siamese_cat`）；不与其它品种合并修订。
> **数值真源**：`预设资产/风格包/cat/siamese_cat/style.json` · schema `ecursor_style_v1`
...
定义 **暹罗猫/高冷型** 的 **12 通道 base_offset / scale_factor**（及几何偏移如有），描述气质与验收。  
...
| label | 暹罗猫/高冷型 |
| species | `cat` |
| notes | 杏仁蓝眼、细长脸，警觉竖耳，瞳孔动态偏大，气质话痨机敏 |
...
| `blink` | 眨眼 | **0.5** | **0.2** | 眼睑开合动态；动态幅度大 |
| `eyebrow` | 眉形整体 | **0.7** | **0.12** | 眉弓压抬；基线偏高 |
| `pupil_scale` | 瞳孔缩放 | **0.65** | **0.3** | 惊恐/聚焦；基线偏高；动态幅度大 |
| `iris_scale` | 虹膜缩放 | **0.52** | **0.15** | 眼内圈大小；接近物种默认 |
| `cornea_bulge` | 角膜鼓胀 | **0.52** | **0.18** | 湿润/受光；接近物种默认 |
| `squint` | 眯眼 | **0.48** | **0.22** | 笑眼/不适；动态幅度大 |
| `brow_raise` | 挑眉 | **0.7** | **0.12** | 警觉/疑问；基线偏高 |
| `lid_upper` | 上睑 | **0.5** | **0.15** | 睁眼度；接近物种默认 |
| `lid_lower` | 下睑 | **0.5** | **0.15** | 眼下缘；接近物种默认 |
| `eye_gloss` | 高光 | **0.48** | **0.05** | 泪膜/湿润感；动态幅度小 |
...
`预设资产/风格包/cat/siamese_cat/style.json`
