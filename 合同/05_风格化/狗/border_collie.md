...
> **类型：个性化 · 狗 · 单项** · id=`border_collie`

> **状态：🧠 脑补初稿（可逐份审定）** — 本文 **只管辖「边牧/聪慧型」**（id=`border_collie`）；不与其它品种合并修订。
> **数值真源**：`预设资产/风格包/dog/border_collie/style.json` · schema `ecursor_style_v1`
...
定义 **边牧/聪慧型** 的 **12 通道 base_offset / scale_factor**（及几何偏移如有），描述气质与验收。  
...
| label | 边牧/聪慧型 |
| species | `dog` |
| notes | 专注侧视感，工作犬钉点，瞳孔动态略大， blink 克制 |
...
| `pupil_y` | 瞳孔垂直 | **0.47** | **0.06** | 抬眼/低眉视线；动态幅度小 |
| `blink` | 眨眼 | **0.42** | **0.16** | 眼睑开合动态；基线偏低 |
| `eyebrow` | 眉形整体 | **0.4** | **0.1** | 眉弓压抬；基线偏低 |
| `pupil_scale` | 瞳孔缩放 | **0.56** | **0.18** | 惊恐/聚焦；接近物种默认 |
| `iris_scale` | 虹膜缩放 | **0.52** | **0.14** | 眼内圈大小；接近物种默认 |
| `cornea_bulge` | 角膜鼓胀 | **0.5** | **0.14** | 湿润/受光；接近物种默认 |
| `squint` | 眯眼 | **0.48** | **0.16** | 笑眼/不适；接近物种默认 |
| `brow_raise` | 挑眉 | **0.42** | **0.08** | 警觉/疑问；基线偏低 |
| `lid_upper` | 上睑 | **0.48** | **0.1** | 睁眼度；接近物种默认 |
| `lid_lower` | 下睑 | **0.48** | **0.1** | 眼下缘；接近物种默认 |
| `eye_gloss` | 高光 | **0.52** | **0.05** | 泪膜/湿润感；动态幅度小 |
...
`预设资产/风格包/dog/border_collie/style.json`
