...
> **类型：个性化 · 狗 · 单项** · id=`husky`

> **状态：🧠 脑补初稿（可逐份审定）** — 本文 **只管辖「哈士奇/凌厉型」**（id=`husky`）；不与其它品种合并修订。
> **数值真源**：`预设资产/风格包/dog/husky/style.json` · schema `ecursor_style_v1`
...
定义 **哈士奇/凌厉型** 的 **12 通道 base_offset / scale_factor**（及几何偏移如有），描述气质与验收。  
...
| label | 哈士奇/凌厉型 |
| species | `dog` |
| notes | 杏仁眼蓝瞳感，少「眉上提」、偏狼相，警觉扫视略多（ ancient breed RAOL 弱） |
...
| 通道 | 中文 | base | scale | 🧠 意图 |
|------|------|------|-------|--------|
| `pupil_x` | 瞳孔水平 | **0.51** | **0.07** | 扫视/回头幅度；接近物种默认 |
| `pupil_y` | 瞳孔垂直 | **0.49** | **0.06** | 抬眼/低眉视线；动态幅度小 |
| `blink` | 眨眼 | **0.44** | **0.16** | 眼睑开合动态；接近物种默认 |
| `eyebrow` | 眉形整体 | **0.38** | **0.06** | 眉弓压抬；基线偏低；动态幅度小 |
| `pupil_scale` | 瞳孔缩放 | **0.58** | **0.22** | 惊恐/聚焦；基线偏高；动态幅度大 |
| `iris_scale` | 虹膜缩放 | **0.5** | **0.14** | 眼内圈大小；接近物种默认 |
| `cornea_bulge` | 角膜鼓胀 | **0.48** | **0.12** | 湿润/受光；接近物种默认 |
| `squint` | 眯眼 | **0.46** | **0.18** | 笑眼/不适；接近物种默认 |
| `brow_raise` | 挑眉 | **0.32** | **0.04** | 警觉/疑问；基线偏低；动态幅度小 |
