...
> **类型：个性化 · 狗 · 单项** · id=`golden_retriever`

> **状态：🧠 脑补初稿（可逐份审定）** — 本文 **只管辖「金毛/温厚型」**（id=`golden_retriever`）；不与其它品种合并修订。
> **数值真源**：`预设资产/风格包/dog/golden_retriever/style.json` · schema `ecursor_style_v1`
...
定义 **金毛/温厚型** 的 **12 通道 base_offset / scale_factor**（及几何偏移如有），描述气质与验收。  
...
| label | 金毛/温厚型 |
| species | `dog` |
| notes | 经典 puppy eyes，内眉上扬湿眼，友善凝视，眼动中等 |
...
金毛：puppy eyes、湿眼、内眉易上提

### 通道级决策（🧠 脑补）

| 通道 | 中文 | base | scale | 🧠 意图 |
|------|------|------|-------|--------|
| `pupil_x` | 瞳孔水平 | **0.5** | **0.05** | 扫视/回头幅度；动态幅度小 |
| `pupil_y` | 瞳孔垂直 | **0.48** | **0.04** | 抬眼/低眉视线；动态幅度小 |
| `blink` | 眨眼 | **0.52** | **0.28** | 眼睑开合动态；动态幅度大 |
| `eyebrow` | 眉形整体 | **0.32** | **0.14** | 眉弓压抬；基线偏低 |
| `pupil_scale` | 瞳孔缩放 | **0.52** | **0.14** | 惊恐/聚焦；接近物种默认 |
| `iris_scale` | 虹膜缩放 | **0.5** | **0.12** | 眼内圈大小；接近物种默认 |
| `cornea_bulge` | 角膜鼓胀 | **0.5** | **0.14** | 湿润/受光；接近物种默认 |
| `squint` | 眯眼 | **0.42** | **0.14** | 笑眼/不适；基线偏低 |
| `brow_raise` | 挑眉 | **0.52** | **0.22** | 警觉/疑问；动态幅度大 |
| `lid_upper` | 上睑 | **0.46** | **0.14** | 睁眼度；接近物种默认 |
| `lid_lower` | 下睑 | **0.5** | **0.12** | 眼下缘；接近物种默认 |
| `eye_gloss` | 高光 | **0.62** | **0.08** | 泪膜/湿润感；基线偏高 |
...
`预设资产/风格包/dog/golden_retriever/style.json`
