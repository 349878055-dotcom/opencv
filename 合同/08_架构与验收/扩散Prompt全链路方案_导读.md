...
**兄弟规范**：[`扩散引擎提示词拼装规范.md`](../07_输出与扩散/扩散引擎提示词拼装规范.md) · [`节奏说明书.md`](../04_通道与先验/节奏说明书.md) · [`pomot合成规范.md`](pomot合成规范.md)
...
送扩散引擎 = 两路，必须同一次 revision 同源：
  ...
  ① 06_工程底膜.mp4     像素控制（R眼 G眉/耳 B瞳孔，150帧）
...
> **路线说明（非 3D）**：本项目 **不做** 全脸 3D mesh/渲染（太慢）；控制侧用 **OpenCV 03 线条 + 02 通道**，外观与质感主靠 **Wan + 客户参考图**。FLAME/矩阵/AU 三维方案 **不进入主链**；style 仍用 **base/scale 表** 改 02，见 [`风格化偏向专篇 §2.5.6`](../05_风格化/风格化偏向专篇_情绪与人格耦合.md#256-非-3d-路线opencv-底膜--wan-扩散本项目选型--必读)。
...
| E | `02` | `render_dog_batch` | `06_工程底膜.mp4` |
...
pkt = SliderPacket.from_dict(json.loads(Path('预设资产/情绪包/dog/委屈·幼犬眼.json').read_text()))
